...
$(document).ready(function () {
	...
	let threshold = 100;
	let carouselOneBlockWidth = $("#carousel")
		.find(".carousel-block")
		.eq(1)
		.outerWidth();
	...
	function translateCarousel(deltaX) {
		carouselInner.style.transform =
			"translateX(" + (offsetX + deltaX) + "px)";
	}

	function snapCarousel() {
		let rounded = Math.round(
			(offsetX % carouselOneBlockWidth) / carouselOneBlockWidth
		);
		let snapped;
		...
		if (rounded === 0) {
			...
			snapped =
				offsetX +
				(carouselOneBlockWidth -
					Math.abs(offsetX % carouselOneBlockWidth)) *
					rounded;
		}
	...
	$(document).ready(function () {
		if (buttons) {
			...
			$(document).on("click", ".carousel-button-left", function () {
				...
				$(carousel)
					.find(".carousel-items")
					.css({ left: "-" + carouselOneBlockWidth * 2 + "px" });
				checkOffset();
				$(carousel)
					.find(".carousel-items")
					.animate({ left: "-" + carouselOneBlockWidth + "px" }, 300);
			});
			...
			$(document).on("click", ".carousel-button-right", function () {
				$(carousel)
					.find(".carousel-items")
					.animate(
						{ left: "-" + carouselOneBlockWidth * 2 + "px" },
						300,
						function () {
							offsetX -= carouselOneBlockWidth;
							checkOffset();
							$(carousel)
								.find(".carousel-items")
								.css({ left: "-" + carouselOneBlockWidth + "px" });
						}
					);
			});
		} else {
